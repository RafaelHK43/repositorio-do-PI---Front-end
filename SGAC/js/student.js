...
/**
 * Busca as submissões do aluno da API do backend
 * @param {number} studentId - ID do aluno
 * @returns {Promise<Array>} Array de submissões ou array vazio em caso de erro
 */
  ...
  try {
    ...
    return Array.isArray(submissoes)
      ? submissoes.map((item) => ({
          ...
          proofFile: item.nomeArquivo || item.proofFile || "",
...
function renderCoursesInElement(element, courses) {
  if (element.matches("select")) {
          ...
          .map(
            (course, index) =>
              `<option value="${course.id}" ${
                String(course.id) === String(selectedValue || courses[0]?.id) ||
    ...
    if (
      courses.length &&
      !courses.some((course) => String(course.id) === String(selectedValue))
    ) {
  ...
  if (element.matches("ul, ol")) {
    element.innerHTML = courses.length
      ? courses
          .map(
            (course) =>
              `<li data-curso-id="${course.id}">${escapeHtml(course.name)}</li>`
          )
          .join("")
...
export async function salvarSubmissao(formElement, navigate) {
  ...
  try {
    ...
    // Recarregar as submissões da API após sucesso
...
/**
 * Obtém o contexto do aluno carregando submissões da API
 * @returns {Promise<Object>} Contexto com dados do aluno e submissões da API
 */
async function getStudentContextAsync() {
  ...
  const activities = await buscarSubmissoesDoAluno(student.id);
  const primaryCourseId = student.courseIds?.[0] || data.courses[0]?.id;
...
export function studentActivitiesPage() {
  ...
  return shell({
    ...
    content: `<div class="page-toolbar"><button class="btn btn-warning" data-open-activity>+ Adicionar Atividade</button></div><section class="student-stats-grid"><div class="student-stat-card warning compact"><span>Pendentes</span><strong>${
      pending.length
    }</strong></div><div class="student-stat-card success compact"><span>Aprovadas</span><strong>${
      approved.length
    }</strong></div><div class="student-stat-card danger compact"><span>Reprovadas</span><strong>${
      rejected.length
    }</strong></div><div class="student-stat-card info compact"><span>Horas Aprovadas</span><strong>${approvedHours}h</strong></div></section><section class="content-card">${
      activities.length
        ? `<div class="table-wrap"><table class="custom-table"><thead><tr><th>Título</th><th>Curso</th><th>Área</th><th>Horas</th><th>Data</th><th>Status</th><th>Ações</th></tr></thead><tbody>${activities
            .map(
              (activity) =>
                `<tr><td><strong>${escapeHtml(
                  activity.title
                )}</strong><div class="table-sub">${escapeHtml(
                  activity.proofFile || "Sem arquivo"
                )}</div></td><td>${escapeHtml(
                  getCourseName(activity.courseId, data)
                )}</td><td>${escapeHtml(
                  getAreaName(activity.areaId, data)
                )}</td><td>${activity.workload}h</td><td>${formatDate(
                  activity.activityDate
                )}</td><td><span class="${statusClass(
                  activity.status
                )}">${escapeHtml(
                  activity.status
                )}</span></td><td class="actions-cell">${
                  activity.status === "pendente"
                    ? `<button class="icon-btn delete" data-delete-activity="${activity.id}">🗑</button>`
...
export function studentAddPage() {
  ...
  return shell({
    ...
    content: `<section class="content-card"><div class="note-box student-note-box"><strong>Antes de enviar:</strong><p>Preencha os dados obrigatórios e adicione o comprovante da atividade.</p></div><div class="page-toolbar left"><button class="btn btn-primary" data-open-activity>Preencher Formulário</button></div></section><section class="content-card"><h3>Conta atual</h3><p>${escapeHtml(
      user.name
    )} · ${escapeHtml(user.email)}</p></section>`,
  });
}
...
export function studentRulesPage() {
  ...
  return shell({
    ...
    subtitle: `Curso atual: ${escapeHtml(
      getCourseName(primaryCourseId, data)
    )}`,
    content: `<section class="student-rules-alert">As regras abaixo orientam o cadastro das atividades e o limite de horas por categoria.</section><section class="cards-grid rules-summary-grid">${courseAreas
      .map(
        (area) =>
          `<div class="summary-card"><strong>${escapeHtml(
            area.name
          )}</strong><span>${area.hour_limit}h máximas</span><p>${escapeHtml(
            area.description
          )}</p></div>`
      )
      .join("")}</section><section class="student-rules-list">${studentRulesReference
      .map(
        (rule) =>
          `<details class="student-rule-card student-rule-accordion"><summary class="student-rule-head"><div class="student-rule-title ${
            rule.color
          }"><span class="student-rule-dot"></span><div><strong>${escapeHtml(
            rule.name
          )}</strong><small>Exemplos de atividades válidas</small></div></div><div class="student-rule-head-right"><span class="student-rule-badge ${
            rule.color
          }">${
            rule.limit
          }h</span><span class="student-rule-arrow">⌄</span></div></summary><div class="student-rule-content"><table class="custom-table student-rule-table"><thead><tr><th>Atividade</th><th>Horas</th></tr></thead><tbody>${rule.items
            .map(
              (item) =>
                `<tr><td>${escapeHtml(item[0])}</td><td>${escapeHtml(
                  item[1]
                )}</td></tr>`
            )
...
export function attachStudentPage(page, { render, navigate }) {
  ...
  // Carregar submissões reais da API e atualizar a renderização se necessário
  if (["student-dashboard", "student-activities"].includes(page)) {
    buscarSubmissoesDoAluno(user.id).then((activities) => {
      // Atualizar o estado local com as activities da API
      const currentData = getData();
      currentData.activities = activities;
      
      // Atualizar apenas a seção de activities sem recarregar toda a página
      const contentSection = document.querySelector("section.content-card");
      if (contentSection && page === "student-activities") {
        const pending = activities.filter((item) => item.status === "pendente");
        const approved = activities.filter((item) => item.status === "aprovada");
        const rejected = activities.filter((item) => item.status === "reprovada");
        const approvedHours = approved.reduce(
        ...
        if (activities.length > 0) {
          ...
          contentSection.innerHTML = emptyState({
            icon: "🏅",
            title: "Nenhuma atividade cadastrada",
            text: "Comece adicionando sua primeira atividade complementar!",
            actionLabel: "Adicionar Atividade",
