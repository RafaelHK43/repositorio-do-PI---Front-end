...
export function shell({
  ...
  const user = getUser();
  return `<div class="app-shell"><header class="topbar"><div class="brand"><div class="brand-logo-wrap"><img src="assets/senac-logo.png" alt="Senac" class="brand-logo-image" /></div><div class="brand-copy"><h1>SGAC</h1><p>${roleLabel}</p></div></div><div class="topbar-actions"><div class="user-chip">${user?.name || "Usuário"}</div><button class="btn btn-outline" data-action="logout">Sair</button></div></header><nav class="topnav">${navItems.map((item) => `<button class="topnav-link ${getPage() === item.page ? "active" : ""}" data-go="${item.page}"><span class="topnav-icon">${item.icon || ""}</span>${item.label}</button>`).join("")}</nav><main class="page-container">${title ? `<section class="page-header compact ${heroClass}"><div class="page-header-icon">${heroTitle ? heroTitle.slice(0, 1) : "•"}</div><div><h2>${title}</h2><p>${subtitle}</p></div></section>` : ""}${heroTitle ? `<section class="hero-card ${heroClass}"><div><h3>${heroTitle}</h3><p>${heroText}</p></div>${heroAction}</section>` : ""}${content}</main><footer class="footer">© 2026 SGAC - Sistema de Gestão de Atividades Complementares</footer></div>`;
}
